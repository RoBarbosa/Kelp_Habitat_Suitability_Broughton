...
###==================================================================
# Function to extrapolate the models to the entire area
predict_maxent_models<- function(models= model_replicates_003, variables= filtered_vars002){
  x <- raster::stack()
  for (i in 1:50){
    Loop.r <- predict(variables, models$models[[1]], progress='text', na.rm=TRUE)
    Loop.r<- raster(Loop.r)
    # plot(Loop.r)
    names(Loop.r)<- paste("mod_rep", i, sep="_")
    x <- stack(x, Loop.r)
    
    print(paste("pred model", i))
  }
  return(x)
}


predictions_mod003<- predict_maxent_models(models= model_replicates_003, variables= filtered_vars003)
...
selected_variable_names002<- variables_importance_table00[which(variables_importance_table00$mean_var_permitImportance>= 2 | variables_importance_table00$mean_var_contrib>= 2),"variable"]
selected_variables002<- variables_importance_table00[which(variables_importance_table00$mean_var_permitImportance>= 2 | variables_importance_table00$mean_var_contrib>= 2),]
selected_variables002<- selected_variables002$variable
subset002<- selected_variables00
...
length(names(filtered_vars002))# 15 instead of 14
...
selected_variable_names003<- variables_importance_table002[which(variables_importance_table002$mean_var_permitImportance>= 5 | variables_importance_table$mean_var_contrib>= 5),"variable"]
...
filtered_vars003 <- raster::subset(stack_vars, subset_ras003)#grep('_GAM', names(my_stack), value = T)
length(names(filtered_vars003))# 11
...
selected_variable_names004<- variables_importance_table003[which(variables_importance_table003$mean_var_permitImportance>= 7 | variables_importance_table003$mean_var_contrib>= 7),"variable"]
variables_importance_table003[which(variables_importance_table003$mean_var_permitImportance>= 7),"variable"]
...
length(names(filtered_vars004))# 4--- 9
...
unique(mod_maxent00$variables_importance$variable)
unique(model_replicates_002$variables_importance$variable)
unique(model_replicates_003$variables_importance$variable)
unique(model_replicates_004$variables_importance$variable)
unique(model_replicates_005$variables_importance$variable)
...
# Create summary table of variables contribution
variables_importance_table004<- model_replicates_004$variables_importance%>%
  group_by(variable)%>%
  summarize(mean_var_contrib= mean(variable_contrib),
            sd_var_contrib= sd(variable_contrib),
            mean_var_permitImportance= mean(permutation_importance),
...
###=============================================================================
### MAXENT ; filtering of variables (>=11% importance for MAXENT) ###############
###=============================================================================
selected_variable_names005<- variables_importance_table004[which(variables_importance_table004$mean_var_permitImportance>= 11 | variables_importance_table004$mean_var_contrib>= 11),"variable"]
variables_importance_table004[which(variables_importance_table004$mean_var_permitImportance>= 11),"variable"]
selected_variable_names005<- selected_variable_names005$variable
subset005<- selected_variable_names005

subset_ras005<- which(names(stack_vars) %in% subset005)
filtered_vars005 <- raster::subset(stack_vars, subset_ras005)
length(names(filtered_vars005))# 5


train_data_subsetVars005<- train_data[c(1:6,which(colnames(train_data)%in% subset005))]
test_data_subsetVars005<- test_data[c(1:6,which(colnames(test_data) %in% subset005)) ]


model_replicates_005<- model_replicates_Varsfunction_v2(replicates_number= 50, 
                                                        training_data= train_data_subsetVars005, 
                                                        variables= filtered_vars005, 
                                                        n_rdompoints= 10000,
                                                        selected_variables= 5,
                                                        testing_data = test_data_subsetVars005,
                                                        pred= T,
                                                        bgd= "variable")


performance_df005<- model_replicates_005$mod_performance
variables_importance_005<- model_replicates_005$variables_importance
  
variables_importance_table005<- variables_importance_005%>%
  group_by(variable)%>%
  summarize(mean_var_contrib= mean(variable_contrib),
            sd_var_contrib= sd(variable_contrib),
            mean_var_permitImportance= mean(permutation_importance),
...
####============================================================================
### Test for differences in performance among models with different number of variables =======
performance_df_00<- mod_maxent00$mod_performance
performance_df_002<- model_replicates_002$mod_performance
performance_df_003<- model_replicates_003$mod_performance
performance_df_004<- model_replicates_004$mod_performance
performance_df_005<- model_replicates_005$mod_performance

performance_df_00$Nvariables<- 17#18#20
performance_df002$Nvariables<-  15#16
performance_df003$Nvariables<- 11
performance_df004$Nvariables<- 9#4
performance_df005$Nvariables<- 5

performance_dfmerged<- rbind(performance_df_00, performance_df002, performance_df003, performance_df004, performance_df005)
...
####============================================================================
### Calculate average prediction and PLOT  maps                          =======
####============================================================================
# ## to calculate a mean and SD rasters

predictions_mod003

mean_maxent003<- calc(predictions_mod003, fun = mean)
# writeRaster(mean_maxent003, "C:/Users/romi_/OneDrive - University of Victoria/Kelp_postdoc/SDMs/outputs_SDMs/maxent_pred_mean_11Vars_50.tif")
mapview::mapview(mean_maxent3001_v2, col.regions = rev(pal(10)), at = seq(0, 1, 0.1), legend = TRUE)

### to calculate raster range
sd_maxent003<-  calc(predictions_mod003, fun = sd)
# writeRaster(sd_maxent003, "C:/Users/romi_/OneDrive - University of Victoria/Kelp_postdoc/SDMs/outputs_SDMs/maxent_pred_SD_11Vars_50Rep.tif")
...
pal_range= Lab.palette <- colorRampPalette(c("red", "orange", "blue"),
                                           space = "Lab")
# 
# # mapview::mapview(mean_maxent003, col.regions = rev(pal(100)), at = seq(0, 1, 0.2), legend = TRUE)
# # mapview::mapview(sd_maxent003, col.regions = rev(pal(20)), at = seq(0, 1, 0.1), legend = TRUE)
...
mean_maxent003_<- calc(x, fun = mean)
...
mapview::mapview(mean_maxent003_, col.regions = rev(pal(10)), at = seq(0, 1, 0.1), legend = TRUE)

### to calculate raster range
# sd_maxent002<-  calc(model_replicates_002$preds, fun = sd)
sd_maxent002<-  calc(x, fun = sd)
writeRaster(sd_maxent003, "C:/Users/romi_/OneDrive - University of Victoria/Kelp_postdoc/SDMs/outputs_SDMs/Sept2024/maxent_pred_SD_11Vars_50.tif")
mapview::mapview(sd_maxent003, col.regions = (palSD(10)), at = seq(0, 0.25, 0.04), legend = TRUE)


# ### Calculate average thresholds ======
# model_replicates_003$mod_performance%>%
...
## to calculate a mean and SD rasters
mean_maxent004<- calc(model_replicates_004$preds, fun = mean)
...
mapview::mapview(mean_maxent004, col.regions = rev(pal(10)), at = seq(0, 1, 0.1), legend = TRUE)

### to calculate raster range
sd_maxent004<-  calc(model_replicates_004$preds, fun = sd)
...
mapview::mapview(sd_maxent004, col.regions = (palSD(10)), at = seq(0, 0.25, 0.04), legend = TRUE)
